...
@app.route('/users')
def get_users():
    # Gateway apenas roteia a requisição
...
```python
@app.route('/users/<id>/complete')
def get_complete_user():
    # Gateway combina dados de múltiplos serviços
    user = get_from_user_service(id)
    orders = get_from_order_service(id)
    return {**user, **orders}
```
...
```python
# Gateway pode converter entre protocolos
# HTTP → gRPC, REST → GraphQL, etc.
```

#### 4. **Request/Response Transformation**

```python
# Gateway pode adicionar metadados
...
Gateway pode ter diferentes implementações por tipo de cliente:

- `gateway-web` - Otimizado para browsers
- `gateway-mobile` - Otimizado para apps mobile
- `gateway-internal` - Para uso interno

### Cross-Cutting Concerns

```python
# Exemplos que podem ser adicionados ao gateway:
...
Para arquiteturas muito grandes, considere Service Mesh:
...
### Do Monolito aos Microsserviços com Gateway:

```
Monolito
   ↓
Microsserviços Independentes (Desafio 4)
   ↓
Microsserviços + API Gateway (Desafio 5) ← Estamos aqui
   ↓
Service Mesh (Produção avançada)
```

### Próximos passos possíveis:

1. **Adicionar mais microsserviços**
